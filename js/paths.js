...
export function hitGPUWorld(wx, wy) {
  // Used by legacy interaction; with SVG we primarily use data attrs
  // but keep for compatibility
  const k = d3.zoomTransform(document.getElementById('sv')).k;
  if (getLOD(k) < 4) return null;
  for (let ni = 0; ni < NODES.length; ni++) {
    const nd = NODES[ni];
    for (let g = 0; g < 8; g++) {
      const col = g % 4, row = Math.floor(g / 4);
      const gx = nd.x + 4 + col * 64, gy = nd.y + [76, 114][row];
      if (wx >= gx && wx <= gx + 54 && wy >= gy && wy <= gy + 26)
        return { nodeIdx: ni, gpuIdx: g, nd };
    }
  }
  return null;
}
...
(function injectDashStyle() {
  if (document.getElementById('path-anim-style')) return;
  const s = document.createElement('style');
  s.id = 'path-anim-style';
  s.textContent = `@keyframes dashMove { to { stroke-dashoffset: -55px; } }
.path-seg { animation: dashMove 0.9s linear infinite; }`;
  document.head.appendChild(s);
})();

// ─── drawPath (SVG) ───────────────────────────────────────────────────────────
export function drawPath() {
  const lPath = d3.select('#l-path');
  lPath.selectAll('*').remove();
  if (!pathMode) return;
  ...
  [pathSrc, pathDst].forEach((p, i) => {
    ...
    const gx = p.nd.x + 4 + p.gpuIdx % 4 * 64;
    const gy = p.nd.y + [76, 114][Math.floor(p.gpuIdx / 4)];
    lPath.append('rect')
      .attr('x', gx - 3).attr('y', gy - 3)
      .attr('width', 60).attr('height', 32).attr('rx', 5)
      .attr('fill', 'none')
      .attr('stroke', col).attr('stroke-width', 2.5)
      .attr('filter', i === 0 ? 'url(#glow-yellow)' : 'url(#glow-pink)')
  ...
  pathResult.segs.forEach(seg => {
    ...
    const al = 10, aw = 5;
    lPath.append('polygon')
      .attr('points',
        `${mx + ux*al},${my + uy*al} ` +
        `${mx - ux*al + uy*aw},${my - uy*al - ux*aw} ` +
        `${mx - ux*al - uy*aw},${my - uy*al + ux*aw}`)
      .attr('fill', col)
      .attr('pointer-events', 'none');
  });
...
// ─── Animation loop ───────────────────────────────────────────────────────────
// With CSS animations on SVG lines, we don't need rAF for dash animation.
// But we keep it to keep drawPath() refreshed when pathMode changes.
